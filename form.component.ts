...
export class FormGeneratorComponent implements OnInit, OnChanges {
  ...
  private updateDataThatHaveChanged(changes: SimpleChanges): void {
    this.formFieldControlNames
      .filter((controlName) =>
        this.formFieldValueHasChanged(changes, controlName)
      )
      .forEach((controlName) => {
        this.myForm
          .get(controlName)
          ?.patchValue(this.predefinedFormValues[controlName]);
      });
  }

  private formFieldValueHasChanged(
  ...
  private createForm(controls: JsonFormControls[]): void {
    for (const control of controls) {
      ...
      this.myForm.addControl(
        ...
        this.fb.control(
          {
            value: this.patchValues(control.name, this.formFieldControlNames),
  ...
  public computeTotalMileage(): void {
    ...
    const personalAnnual = this.myForm.value.personalAnnual
      ? parseInt(this.myForm.value.personalAnnual, 0)
      : 0;
    const businessAnnual = this.myForm.value.businessAnnual
      ? parseInt(this.myForm.value.businessAnnual, 0)
      : 0;
    this.myForm
      .get('businessContract')
      ?.patchValue(this.computeContractValue(businessAnnual));
    this.myForm
      .get('personalContract')
      ?.patchValue(this.computeContractValue(personalAnnual));
    this.myForm.get('totalAnnual')?.patchValue(personalAnnual + businessAnnual);
    this.myForm
      .get('totalContract')
      ?.patchValue(
        this.myForm.get('businessContract')?.value +
          this.myForm.get('personalContract')?.value
      );
